...
const ServicesList = ({ themeStyles }) => {
  ...
  const [services, setServices] = useState([
    ...
    { category: "Pumps Services", name: "Water Pump Installation - Borehole Depth" },
    { category: "Pumps Services", name: "Water Pump Installation - Height of Tank" },
    ...
    { category: "Plumbing Services", name: "ABS - Acrylonitrile Butadiene Styrene" },
  ...
  return (
    <div className="pb-40 px-5 py-7 w-full h-screen overflow-y-auto" style={background}>
      <div className="max-w-4xl mx-auto bg-gray-100 shadow-md rounded-lg px-8 pt-6 pb-8 mb-4">
        <h2 className="text-gray-800 text-2xl font-bold text-center mb-4">OUR SERVICES LIST</h2>
        ...
        {categories.map((category) => (
          <div className="mb-6 bg-white shadow-md rounded-lg p-4" key={category}>
            <h3 className="text-gray-700 font-bold text-lg mb-2">{category}</h3>
            <ul className="text-black py-1 px-2">
              {filteredServices
                .filter((service) => service.category === category)
                .map((service, index) => (
                  <li key={index} className="bg-gray-100 shadow-md rounded-lg p-2 mb-2 flex justify-between items-center">
                    {service.name}
                    <button
