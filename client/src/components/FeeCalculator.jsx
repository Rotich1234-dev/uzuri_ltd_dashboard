...
const FeeCalculator = ({ ThemeStyles }) => {
  ...
  const fetchClientServices = async (clientId) => {
    try {
      const response = await fetch("http://127.0.0.1:8080/api/admin/routes/services");
      if (!response.ok) {
        throw new Error(`Error fetching client services: ${response.statusText}`);
      }
      const data = await response.json();
      setDrillingServices(data.drillingServices || []);
      setPumpTypes(data.pumpTypes || []);
      setPipeTypes(data.pipeTypes || []);
      setPumpDepth(data.pumpDepth || "");
      setPumpHeight(data.pumpHeight || "");
      setPipeDiameter(data.pipeDiameter || "");
      setPipeLength(data.pipeLength || "");
      setNumberOfOutlets(data.numberOfOutlets || "");
  ...
  const handleSubmit = (e) => {
    ...
    if (
      clientCategory &&
      clientCategory.surveyFee &&
      clientCategory.localAuthorityFee
    ) {
      cost += clientCategory.surveyFee + clientCategory.localAuthorityFee;
      breakdown.push({
        label: `Survey Fee (${clientCategory.category})`,
        amount: clientCategory.surveyFee,
      });
      breakdown.push({
        label: `Local Authority Fee (${clientCategory.category})`,
        amount: clientCategory.localAuthorityFee,
      });
    }

    drillingServices.forEach((service) => {
      const serviceCost = serviceData[0]?.drillingservices.find((s) => s.drill_type === service)?.downpayment;
      if (serviceCost) {
        cost += serviceCost;
        breakdown.push({ label: service, amount: serviceCost });
      }
    });

    pumpTypes.forEach((service) => {
      const serviceCost = serviceData[2]?.pumpservices.find((s) => s.pump_type === service)?.cost;
      if (serviceCost) {
        cost += serviceCost;
        breakdown.push({ label: service, amount: serviceCost });
      }
    ...
    pipeTypes.forEach((pipe) => {
      if (pipeTypes[pipe]) {
        cost += pipeTypes[pipe];
        breakdown.push({ label: pipe, amount: pipeTypes[pipe] });
      }
    });

    if (pipeDiameter) {
      const diameterCost =
        parseFloat(pipeDiameter) * pipeDiameter["Pipe Diameter"];
      cost += diameterCost;
      breakdown.push({
        label: `Pipe Diameter (${pipeDiameter} inches)`,
        amount: diameterCost,
      });
    }

    if (pipeLength) {
      const lengthCost = parseFloat(pipeLength) * pipeLength["Pipe Length"];
      cost += lengthCost;
      breakdown.push({
        label: `Pipe Length (${pipeLength} mm)`,
        amount: lengthCost,
      });
    }
    ...
    if (numberOfOutlets) {
      const outletsCost =
        parseFloat(numberOfOutlets) * numberOfOutlets["Number Of Outlets"];
      cost += outletsCost;
      breakdown.push({
        label: `Number Of Outlets (${numberOfOutlets})`,
        amount: outletsCost,
      });
    }
    ...
    //     pumpDepth.find((d) => pumpDepthValue <= d.maxPumpDepthHeight)
    //       .costPerMeter * pumpDepthValue;
    //   cost += pumpDepth;
    //   breakdown.push({
    //     label: `Pump Depth (${pumpDepth} meters)`,
    //     amount: pumpDepth,
    //   });
    // }
  
    if (pumpDepth) {
      const pumpDepthCost = parseFloat(pumpDepth) * pumpDepth["Pump Depth"];
      cost += pumpDepthCost;
      breakdown.push({
        label: `Pump Depth (${pumpDepth} mm)`,
        amount: pumpDepthCost,
      });
    }

    if (pumpHeight) {
      const pumpHeightCost = parseFloat(pumpHeight) * pumpHeight["Pump Height"];
      cost += pumpHeightCost;
      breakdown.push({
        label: `Pump Height (${pumpHeight} mm)`,
        amount: pumpHeightCost,
      });
    }
    ...
    //   const pumpHeightCost =
    //     pumpHeightCost.find(
    //       (d) => pumpHeightValue <= d.maxPumpDepthHeight
    //     ).costPerMeter * pumpHeightValue;
    //   cost += pumpHeightCost;
    //   breakdown.push({
    //     label: `Pump Height (${pumpHeight} meters)`,
    //     amount: pumpHeightCost,
    //   });
    // }

    otherServices.forEach((service) => {
      if (otherServices[service]) {
        cost += otherServices[service];
        breakdown.push({ label: service, amount: otherServices[service] });
      }
    });

    if (tankCapacity) {
      const tankCapacityCost =
        parseFloat(tankCapacity) * tankCapacity;
      cost += tankCapacityCost;
      breakdown.push({
        label: `Tank Capacity (${tankCapacity} liters)`,
        amount: tankCapacityCost,
      });
    }

    const tax = cost * 0.16;
  const totalAmountWithTax = cost + taxAmount;

// const tax = cost * 0.16;
// const totalAmountWithTax = cost + tax;
...
<div
  ...
  style={background}
>
  <h1 className="text-gray-800 text-2xl font-bold p-3 text-center">
    Fee Calculator
  </h1>
